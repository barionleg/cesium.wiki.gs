...
* Layer imagery from multiple sources, including WMS, TMS, OpenStreetMap, Bing Maps, ArcGIS MapServer, and standard image files.  Each layer can be alpha blended with the layers below it, and its brightness, contrast, and gamma can be dynamically changed.
* Draw vector data from KML ([supported elements](https://github.com/AnalyticalGraphicsInc/czml-writer/wiki/Kml)), ESRI Shapefiles, and WebGL Globe JSON.

...
1. Run tests against the release `npm run test -- --failTaskOnError --release`. Test **in all browsers** with the `--browsers` flag (i.e. `--browsers Firefox`). Alternatively, test with the browser Spec Runner by browsing to http://localhost:8080/Specs/SpecRunner.html?built=true&release=true.
